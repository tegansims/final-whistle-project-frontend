...
import { Segment, Image, SegmentGroup, Header } from "semantic-ui-react";
...
class PlayersShowPage extends React.Component {
  ...
  componentDidMount() {
    if (localStorage.getItem("token")) {
      API.player(this.props.match.params.id).then(player =>
  ...
  render() {
    ...
    return (
      <>
        ...
        <Segment>
          <Header className="center aligned Header">{player.name}</Header>
        </Segment>
      </>
    );
  }
